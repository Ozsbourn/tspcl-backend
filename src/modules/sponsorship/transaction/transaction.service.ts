	...
	public async makePayment(user: User, planId: string) {
		...
		if (user.id === plan.channel?.id) {
			throw new ConflictException(
				"You can not apply for sponsorship for yourself"
			);
		}
		...
		if (existingSubscription) {
			throw new ConflictException(
				"You have already applied the sponsorship for this channel"
			);
		}
		...
		const successUrl = `${this.configService.getOrThrow<string>("ALLOWED_ORIGIN")}/success?price=${plan.title}&username=${encodeURIComponent(plan.channel?.username as string)}`;
		const cancelUrl =
