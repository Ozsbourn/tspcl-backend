...
export class TelegramService extends Telegraf {
	...
	public async onStart(@Ctx() ctx: any) {
		...
		if (token) {
			...
			if (!authToken) {
				await ctx.reply(MESSAGES.tokenNotFound);
			}

			const hasExpired =
				new Date(authToken?.expiresIn as Date) < new Date();
			if (hasExpired) {
				await ctx.reply(MESSAGES.invalidToken);
			}

			await this.connectTelegram(authToken!.userId as string, chatId);
	...
	public async onFollows(@Ctx() ctx: Context) {
		...
		if (user && follows.length) {
			...
			const message = `<b>Channels you are following to:<b>\n\n${followsList}`;
			await ctx.replyWithHTML(message);
		} else {
			await ctx.replyWithHTML("<b>You do not have a followings</b>");
		}
	}
	...
	public async sendDeactivateAcountToken(
		chatId: string,
		token: string,
		metadata: SessionMetadata
	) {
