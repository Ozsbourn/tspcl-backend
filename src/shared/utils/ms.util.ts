...
export function ms(str: StringValue) : number {
	if (typeof str !== 'string' || str.length === 0 ||  str.length > 100) {
		throw new Error('Value provided to ms() must be a string with length betwween 1 and 99.');
	} 
	...
	switch (type) {
		...
		default:
			throw new Error(`Unit of time ${type} was matched, but there isn't related case. Please check entered data.`);
	}	
}